...
function queryitem()
{
    ...
    window.location = "orderlist.do?type="+type+"&key="+key;
}
...
<table width="98%" border="0">
  <tr>
    <td width="4">&nbsp;</td>
    <td height="25" align="left"  style="border-bottom: dashed 1px"><img src="../images/dot_process.gif" width="7" height="7" />&nbsp;订房管理 &gt;&gt;&nbsp;历史订单列表</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
		    ...
		    <table width="100%">
              <tr>
	            ...
	            <c:if test="${Pagescoll!=0&&Pagescoll!=1}">
	              <a href="orderlist.do?Page=1&type=${type}&key=${key}">首页</a>
                  <a href="orderlist.do?Page=${Pageno-1}&type=${type}&key=${key}">上一页</a>
	            </c:if>
	            <c:if test="${Pagescoll!=0&&Pagescoll!=3}">
	              <a href="orderlist.do?Page=${Pageno+1}&type=${type}&key=${key}">下一页</a>
                  <a href="orderlist.do?Page=${Totalpage}&type=${type}&key=${key}">尾页</a>
	            </c:if>
	            选择页数：
	            <select onchange='javascript:window.location="orderlist.do?type=${type}&key=${key}&Page=" + this.value;'>
