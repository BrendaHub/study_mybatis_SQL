...
function queryitem()
{
    ...
    window.location = "order1.do?type="+type+"&key="+key+"&cardno="+cardno;
}
...
<table width="98%" border="0">
  ...
  <tr>
    ...
    <td height="25" align="left"  style="border-bottom: dashed 1px">
      ...
      输入身份证号：<input type="text" name="cardno" id="cardno" value="${cardno}" size="20" maxlength="50" />
      <input type="submit" value="查询" class="btn_1" />  
    </td>
  </tr>
  </form>
</table>
<form action="delorder1.do" method="post" name="form1" target="main" id="form1" onsubmit="return CheckForm()">
<table width="98%" border="0">
  <tr>
    <td width="4">&nbsp;</td>
    ...
    <td height="25" align="left" valign="top"  style="line-height:150%">
      <table width="98%" border="0" cellpadding="0" cellspacing="1" bgcolor="#B1C3D9" id=table1 style="margin-top:5px;">
        <tr class="line_bottom">
          ...
          <th height="31" align="center" valign="middle" bgcolor="#FFFFFF" class="line_bottom">是否退订</th>
          <th height="31" align="center" valign="middle" bgcolor="#FFFFFF" class="line_bottom">是否退房</th>
          <th colspan="3" align="center" valign="middle" bgcolor="#FFFFFF" class="line_bottom">操作</th>
        </tr>
        ...
        <tr class="line_bottom" bgcolor='${colors}'>
          ...
          <td height="27" align="center" valign="middle"  class="line_bottom" scope="row">
            <c:if test="${item.client.isbad=='1'}"><a href="showclient.do?id=${item.client.id}&type=1">${item.client.name}(犯罪人员)</a></c:if>
            <c:if test="${item.client.isbad=='0'}"><a href="showclient.do?id=${item.client.id}&type=2">${item.client.name}</a></c:if>
          </td>
          ...
          <td height="27" align="center" valign="middle"  class="line_bottom" scope="row">
            <c:if test="${item.done=='0'}">否</c:if>
            <c:if test="${item.done=='1'}">是</c:if>
          </td>
          <td height="27" align="center" valign="middle" width="4%">
            <c:if test="${item.isused=='0'&&item.done=='0'}"><a href="returnorder.do?id=${item.id}&type=1">退订</a></c:if>
            <c:if test="${item.isused=='1'&&item.done=='1'}">&nbsp;</c:if>
          </td>
          <td height="27" align="center" valign="middle" width="4%">
            <c:if test="${item.done=='0'&&item.isused=='0'}"><a href="returnorder.do?id=${item.id}&type=2">退房</a></c:if>
            <c:if test="${item.done=='1'||item.isused=='1'}">&nbsp;</c:if>
          </td>
		    ...
		    <table width="100%">
              <tr>
	            ...
	            <c:if test="${Pagescoll!=0&&Pagescoll!=1}">
	              <a href="order1.do?Page=1&type=${type}&key=${key}&cardno=${cardno}">首页</a>
                  <a href="order1.do?Page=${Pageno-1}&type=${type}&key=${key}&cardno=${cardno}">上一页</a>
	            </c:if>
	            <c:if test="${Pagescoll!=0&&Pagescoll!=3}">
	              <a href="order1.do?Page=${Pageno+1}&type=${type}&key=${key}&cardno=${cardno}">下一页</a>
                  <a href="order1.do?Page=${Totalpage}&type=${type}&key=${key}&cardno=${cardno}">尾页</a>
	            </c:if>
	            选择页数：
	            <select onchange='javascript:window.location="order1.do?type=${type}&key=${key}&cardno=${cardno}&Page=" + this.value;'>
